...
let TOLERANCE = 0.1 // the margin of error allowed between doors on either
...
public class Graph: CustomStringConvertible {
    ...
    func nodifyRooms(){
        for room in building.rooms {
            nodes[room.name] = Node(name: room.name, room: room)
    ...
    func createPaths(){
        ...
        for room in rooms {
            var borderingRooms = [(String, Door)]()
    ...
    func addPathsToNode(currNode: Node, borderingRooms: [(String, Door)] ) {
        for roomInfo in borderingRooms {
            currNode.addPath(startNode: currNode, endNode: nodes[roomInfo.0]!, pathDoor: roomInfo.1)
        }
    }
    
    
    //For a door, searches all other doors besides for the room that the door is in
    func findMatches(currDoor: Door, currRoom: Room) -> (String, Door) {
        var matchingRoomString: String = ""
        var matchingRoomDoor: Door = currDoor //just a placeholder that will be overwritten if possible
        for room in building.rooms {
            ...
            for door in room.doors {
                if isMatchingDoor(currDoor: currDoor, newDoor: door) {
                    ...
                    matchingRoomDoor = door
                }
                
            }
        }
        return (matchingRoomString, matchingRoomDoor)
    }
